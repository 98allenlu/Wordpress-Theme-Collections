...
$ev_purchase_url = get_field( 'ev_purchase_url', $event_id );
$ev_signup = get_field( 'ev_signup', $event_id );
$ev_gallery = get_field( 'ev_gallery', $event_id );
//$ev_category = tribe_get_event_taxonomy( $event_id );
...
<div id="tribe-events-content" class="tribe-events-single container-fluid mt-2">
	...
	<div class="row wrapper bottom">
			...
			<?php while ( have_posts() ) :  the_post(); ?>
				<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
					...
					<?php if ( tribe_event_featured_image( $event_id, 'full', false ) ) : ?>
						...
						<figure class="text-center bg-lighter wrapper-sm bottom <?php echo $fill; ?>">
				...
				<?php //exit after one loop to avoid repeating content on single w/o date in slug
				break; ?>
		...
		<div class="col-md-5 col-lg-4 content-area" id="secondary">
			...
			<?php } elseif( $ev_purchase_url ) { ?>

				<div class="mt-3">
					<a href="<?php echo esc_url( $ev_purchase_url ); ?>" target="_blank" class="btn btn-secondary d-block">Book now</a>
				</div>

			<?php } ?>

			<?php if( $ev_signup['url'] && $ev_signup['text'] ) { ?>

				<div class="mt-3">
					<a href="<?php echo esc_url( $ev_signup['url'] ); ?>" target="_blank" class="btn btn-secondary d-block"><?php echo esc_html( $ev_signup['text'] ); ?></a>
				</div>
